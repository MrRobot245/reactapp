...
var blobContent= require('./test.blob');
...
class App extends Component {
	...
	loadCSVtoState(input){
		...
		for(var pre=0; pre < 11 ; pre++)
		{
			...
			if(pre===3)
			{
				...
				{
					if(this.state.startDate.format("MMM Do YYYY") < moment().startOf('month').format("MMM Do YYYY"))
					{
						this.setState({
							ranges: {
								'Today': [moment(), moment()],
					...
					else if(this.state.startDate.format("MMM Do YYYY") < moment().subtract(1, 'month').format("MMM Do YYYY"))
					{
						this.setState({
							ranges: {
	...
	testinput(event)
	{
		...
		// var file=new File(["https://github.com/MrRobot245/reactapp/raw/master/csvExample.csv"],"csvExample.csv");
		// console.log(file);
		// console.log
	...
	render() {
		...
		return (
			...
			<br/>
			{/*<button onClick={this.testinput} className='btn'>Test Data</button>*/}
			<br/>
